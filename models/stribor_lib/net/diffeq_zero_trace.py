...
class ZeroTraceEquivariantEncoder(nn.Module):
    ...
    Args:
        ...
        out_dim (int): Output size
    """
    def __init__(self, in_dim, hidden_dims, out_dim, pooling, context_dim, **kwargs):
        super().__init__()
        ...
        self.set_emb = st.net.DiffeqMLP(in_dim + context_dim + 1, hidden_dims, out_dim)
...
class DiffeqZeroTraceDeepSet(nn.Module):
    ...
    Args:
        ...
        return_log_det_jac (bool, optional): Whether to return the log-Jacobian
            diagonal values (always zero). Default: True
    """
    def __init__(self, in_dim, hidden_dims, out_dim, context_dim=None, pooling='max', return_log_det_jac=True, **kwargs):
        super().__init__()
        context_dim=None
        self.elementwise = st.net.MADE(in_dim, hidden_dims, out_dim, context_dim, return_per_dim=True)
        self.interaction = ZeroTraceEquivariantEncoder(in_dim, hidden_dims, out_dim // in_dim, pooling, context_dim)
        self.return_log_det_jac = return_log_det_jac
...
#             x = torch.cat([x, latent], -1)
        ...
        # y = self.elementwise(x, latent) + self.interaction(t, torch.cat([x, latent], -1), mask=mask)
